...
import {
  ...
  XCircle, CheckCircle, Tag, Pause, UserCheck, UserX, Eye,
  Heart, Mail, Clock, UserPlus, Image, MousePointer, List,
  Globe, LogOut, Reply
} from 'lucide-react';

// Channel-specific triggers
// Note: Time-based triggers (invite_ignored, no_response) removed - use Condition with waitTime instead
const CHANNEL_TRIGGERS = {
  linkedin: [
    { event: 'invite_sent', label: 'Convite Enviado', description: 'Ao enviar convite', icon: Send },
    { event: 'message_received', label: 'Msg Recebida', description: 'Lead responde', icon: MessageCircle },
    { event: 'profile_viewed', label: 'Perfil Visto', description: 'Lead viu perfil', icon: Eye },
    { event: 'post_engagement', label: 'Engajamento', description: 'Curtiu/comentou', icon: Heart },
    { event: 'inmail_received', label: 'InMail', description: 'Resposta InMail', icon: Mail }
  ],
  whatsapp: [
...
const Sidebar = ({ channel, collapsed = false, onToggle }) => {
  // Build categories with channel-specific triggers
  const nodeCategories = useMemo(() => {
    const triggers = CHANNEL_TRIGGERS[channel] || CHANNEL_TRIGGERS.linkedin;
