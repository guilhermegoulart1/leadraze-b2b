    ...
    <div className="relative">
      ...
      {showDropdown && (
        <>
          ...
          />
          <div className="absolute right-0 top-full mt-2 w-72 bg-gray-800 rounded-xl shadow-xl shadow-gray-900/50 border border-gray-700 z-20 overflow-hidden">
            {/* Header */}
            <div className="px-4 py-3 bg-gray-800/50 border-b border-gray-700">
              <h3 className="text-sm font-semibold text-gray-100">Uso de Créditos</h3>
            </div>
              ...
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${
                    (credits.ai?.percentUsed || 0) >= 90 ? 'bg-red-900/30' :
                    (credits.ai?.percentUsed || 0) >= 70 ? 'bg-amber-900/30' : 'bg-green-900/30'
                  }`}>
                    <Zap className={`w-4 h-4 ${
                      (credits.ai?.percentUsed || 0) >= 90 ? 'text-red-400' :
                      (credits.ai?.percentUsed || 0) >= 70 ? 'text-amber-400' : 'text-green-400'
                    }`} />
                  </div>
                  <div>
                    <div className="text-sm font-medium text-gray-100">Créditos de IA</div>
                    <div className="text-xs text-gray-400">Agentes automáticos</div>
                  </div>
                </div>
            ...
            <div className="p-4 border-b border-gray-700">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${
                    (credits.gmaps?.percentUsed || 0) >= 90 ? 'bg-red-900/30' :
                    (credits.gmaps?.percentUsed || 0) >= 70 ? 'bg-amber-900/30' : 'bg-green-900/30'
                  }`}>
                    <MapPin className={`w-4 h-4 ${
                      (credits.gmaps?.percentUsed || 0) >= 90 ? 'text-red-400' :
                      (credits.gmaps?.percentUsed || 0) >= 70 ? 'text-amber-400' : 'text-green-400'
                    }`} />
                  </div>
                  <div>
                    <div className="text-sm font-medium text-gray-100">Google Maps</div>
                    <div className="text-xs text-gray-400">Busca de leads</div>
                  </div>
                </div>
                <div className="text-right">
                  <div className={`text-lg font-bold ${
