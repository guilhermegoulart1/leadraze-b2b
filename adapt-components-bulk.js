...
function adaptComponent(filename) {
  ...
  // Cleanup: Remove duplicate dark mode classes
  content = content.replace(/(dark:[a-z-]+\/?\d*)(dark:[a-z-]+\/?\d*)/g, '$1');

  fs.writeFileSync(filePath, content, 'utf8');

  console.log(`✅ ${filename} adapted (${originalLength} → ${content.length} chars)`);
}

console.log('🔄 Adapting UI components for dark mode...\n');
console.log('='.repeat(50));

components.forEach(adaptComponent);

console.log('\n' + '='.repeat(50));
console.log('✅ All UI components adapted successfully!');
