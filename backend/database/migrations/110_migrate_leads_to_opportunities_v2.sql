-- Migration 110 v2: Migrar leads para opportunities (CORRIGIDO)
...
-- (Sem custom_fields - campo não existe na tabela leads)
-- =====================================================
DO $$
DECLARE
  v_lead RECORD;
...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      -- 3. Criar OPPORTUNITY (sem custom_fields)
...
-- (usando 'title' em vez de 'content')
-- =====================================================
...
SELECT
  ci.id,
  ci.checklist_id,
  ci.title,  -- 'title' na tabela original, 'content' na nova
  ci.is_completed,
  ci.completed_by,
  ci.completed_at,
  ci.position,
